...
public class ObjectJava8 {
	...
	public static void main(String[] args) {
		...
		/*
		 * hamStream.forEach(ham->{ System.out.println(ham.age); });
		 * hamStream.filter(ham->{ return ham.age>35; }).forEach(eachHam->{
		...
		List<Hammer> sortedList = hamStream.sorted(hamStringComp).collect(
				Collectors.toList());

		/*
		 * To print use hamStream.sorted(hamStringComp).forEach(hamm->{
		 * System.out.println(hamm.age+" "+hamm.name); });
		 */

		for (Hammer h : sortedList) {
			System.out.println(h.age + " " + h.name);
		}

		/*
		 * String [] streams={"Vijith","Victor"}; Stream<String>
		 * s=Stream.of(streams);
		 * 
		 * s.filter(str->{ return str.startsWith("V");
